package com.luoyuhang.androidutils.http;
...
import com.luoyuhang.androidutils.utils.ToastUtils;
...
public class HttpClient {
    ...
    public static boolean isNetworkAvailable() {
        try {
            ConnectivityManager connectivityManager = (ConnectivityManager) AppManager.getInstance().currentActivity().getSystemService(Context.CONNECTIVITY_SERVICE);
    ...
    public static void get(String url, Map<String, String> param, final HttpResponseHandler handler) {
        if (!isNetworkAvailable()) {
            Context context= AppManager.getInstance().currentActivity().getBaseContext();
            ToastUtils.showToast(context,R.string.no_network_connection_toast);
            return;
        }
    ...
    public static void post(String url, Map<String, String> param, final HttpResponseHandler handler) {
        if (!isNetworkAvailable()) {
            Toast.makeText(AppManager.getInstance().currentActivity(), R.string.no_network_connection_toast, Toast.LENGTH_SHORT).show();
            return;
        }
        String paramStr = "";
        if(param != null && param.size() > 0) {
